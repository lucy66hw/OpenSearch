...
public class TermsAggregationBuilderConverter implements AggregationBuilderProtoConverter {
    ...
    private static String convertExecutionHint(TermsAggregationExecutionHint hint) {
        switch (hint) {
            ...
            case TERMS_AGGREGATION_EXECUTION_HINT_GLOBAL_ORDINALS_HASH:
                return "global_ordinals_hash";
            case TERMS_AGGREGATION_EXECUTION_HINT_GLOBAL_ORDINALS_LOW_CARDINALITY:
                return "global_ordinals_low_cardinality";
            default:
                throw new IllegalArgumentException("Unsupported execution hint: " + hint);
        }
    }
    ...
    private static SubAggCollectionMode convertCollectMode(TermsAggregationCollectMode mode) {
        switch (mode) {
            ...
            default:
                throw new IllegalArgumentException("Unsupported collect mode: " + mode);
        }
    }
    ...
    private static IncludeExclude convertInclude(TermsInclude include) {
        switch (include.getTermsIncludeCase()) {
            case REGEXP:
                return new IncludeExclude(include.getRegexp(), null);
            case TERMS:
                String[] includeTerms = include.getTerms().getStringArrayList().toArray(new String[0]);
                return new IncludeExclude(includeTerms, null);
            case PARTITION:
                return new IncludeExclude(include.getPartition().getPartition(), include.getPartition().getNumPartitions());
            default:
                throw new IllegalArgumentException("Unsupported include type: " + include.getTermsIncludeCase());
        }
    }
    ...
    private static IncludeExclude convertExclude(TermsExclude exclude) {
        switch (exclude.getTermsExcludeCase()) {
            case REGEXP:
                return new IncludeExclude(null, exclude.getRegexp());
            case TERMS:
                String[] excludeTerms = exclude.getTerms().getStringArrayList().toArray(new String[0]);
                return new IncludeExclude(null, excludeTerms);
            default:
                throw new IllegalArgumentException("Unsupported exclude type: " + exclude.getTermsExcludeCase());
        }
    }
