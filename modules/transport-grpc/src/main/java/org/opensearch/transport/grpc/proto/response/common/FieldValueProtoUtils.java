...
public class FieldValueProtoUtils {
    ...
    public static void toProto(Object javaObject, FieldValue.Builder fieldValueBuilder) {
        ...
        if (javaObject instanceof String) {
            ...
            num.setInt32Value((Integer) javaObject);
            fieldValueBuilder.setGeneralNumber(num.build());
        } else if (javaObject instanceof Long) {
            org.opensearch.protobufs.GeneralNumber.Builder num = org.opensearch.protobufs.GeneralNumber.newBuilder();
            num.setInt64Value((Long) javaObject);
            fieldValueBuilder.setGeneralNumber(num.build());
        } else if (javaObject instanceof Double) {
            org.opensearch.protobufs.GeneralNumber.Builder num = org.opensearch.protobufs.GeneralNumber.newBuilder();
            num.setDoubleValue((Double) javaObject);
            fieldValueBuilder.setGeneralNumber(num.build());
        } else if (javaObject instanceof Float) {
            org.opensearch.protobufs.GeneralNumber.Builder num = org.opensearch.protobufs.GeneralNumber.newBuilder();
            num.setFloatValue((Float) javaObject);
            fieldValueBuilder.setGeneralNumber(num.build());
        } else if (javaObject instanceof Boolean) {
            fieldValueBuilder.setBool((Boolean) javaObject);
        } else if (javaObject instanceof Enum<?>) {
            fieldValueBuilder.setString(javaObject.toString());
        } else if (javaObject instanceof BigInteger) {
            BigInteger bi = (BigInteger) javaObject;
